...
class ARMultiAnnDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y_mean = torch.stack(y_list).mean(0) 
        ...
        return x_vars, y_list, y_cons, y_mean

def collate_var(batch):
    xs, y_lists, y_cons, y_mean = zip(*batch)
    return torch.stack(xs), list(y_lists), torch.stack(y_cons), torch.stack(y_mean)
...
def main(cfg):
    ...
    for epoch in range(cfg.epochs):
        ...
        for x_vars, _, y_cons in tqdm(train_dl, desc=f"Train {epoch:02d}", ncols=100):
            ...
            with torch.amp.autocast(device_type="cuda"):
                ...
                # loss = loss_procrustes #loss_jaccard + (0.1*loss_procrustes)
    ...
    torch.save(seg.state_dict(), out_dir / "unet_BCE_Dice.pt")
